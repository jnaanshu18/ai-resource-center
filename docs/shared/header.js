/**
 * Temporary DCS site header stand-in.
 *
 * Mounts into #dcs-site-header.
 * When embedding into the real company site, delete this file + header.css
 * and replace the mount block in index.html with the company header.
 *
 * Asset paths assume this script lives at docs/shared/header.js
 * (so assets resolve to docs/shared/assets/...).
 */
(function () {
        ...
        <div class="dcs-header__inner">
          ...
          <nav aria-label="Primary">
            <ul class="dcs-header__nav">
              ${NAV.map((item) => `
                <li><a href="${item.href}"${item.current ? ' aria-current="page"' : ""}>${item.label}</a></li>
              `).join("")}
            </ul>
          </nav>

          <div class="dcs-header__actions">
            <a
              class="dcs-btn dcs-btn--upwork"
              href="https://www.upwork.com/agencies/dailycodesolutions/"
              target="_blank"
